...
class Video2ImageTranscoder(ITranscoder):
    ...
    options = [IntOption(name='second', description='The second from which frame is to be extracted', default=0),
    ...
    def transcode(self, dest_path, file_descr,
        ...
        try:
            ...
            pro = subprocess.Popen(['ffmpeg', '-ss', str(options['second']),
                '-i', file_descr.file.filename, '-t', '1', '-r', '1', tmp.name, '-y' ])
